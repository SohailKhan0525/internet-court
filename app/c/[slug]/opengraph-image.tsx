...
export default async function Image({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return new ImageResponse(<div style={{background:'#171511',color:'#fffdf7',width:'100%',height:'100%',display:'flex',flexDirection:'column',justifyContent:'space-between',padding:72,fontFamily:'sans-serif'}}><div style={{display:'flex',fontSize:28,letterSpacing:3,color:'#bdb5a5'}}>INTERNET COURT</div><div style={{display:'flex',flexDirection:'column',gap:24}}><div style={{fontSize:58,fontWeight:700,lineHeight:1.08}}>{title}</div><div style={{fontSize:28,color:'#d2c9b9',lineHeight:1.3}}>{argument.slice(0,220)}{argument.length > 220 ? '…' : ''}</div><div style={{fontSize:30}}>{verdict} · {total} {total === 1 ? 'vote' : 'votes'}</div></div><div style={{fontSize:24,color:'#8a8172'}}>Someone is wrong. Probably you.</div></div>, { width:1200, height:630 });
}
